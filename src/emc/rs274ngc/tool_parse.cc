...
static bool scan_old_style(
    ...
    int scanned, toolno, index, pocket, orientation;
       ...
       (scanned == 8 || scanned == 9))
    {
        if(!random_toolchanger) {
            continuous_index++;
            if(continuous_index >= CANON_POCKETS_MAX) {
        ...
        if (index < 0 || index >= CANON_POCKETS_MAX) {
            ...
            /* lathe tool */
            toolTable[index].toolno = toolno;
            toolTable[index].pocket = pocket;
            toolTable[index].offset.tran.z = zoffset;
            toolTable[index].offset.tran.x = xoffset;
            toolTable[index].diameter = diameter;

            toolTable[index].frontangle = frontangle;
            toolTable[index].backangle = backangle;
            toolTable[index].orientation = orientation;
            if(ttcomments && scanned == 9) strcpy(ttcomments[index], comment);
            return true;
        }
    }
           ...
           (scanned == 4 || scanned == 5))
    {
        ...
        if (index < 0 || index >= CANON_POCKETS_MAX) {
            ...
            toolTable[index].frontangle = toolTable[index].backangle = 0.0;
            toolTable[index].offset.tran.x = 0.0;
            toolTable[index].orientation = 0;
            if(ttcomments && scanned == 5) strcpy(ttcomments[index], comment);
            return true;
        }
    }
    return false;
             ...
             int random_toolchanger)
{
    ...
    */
    ...
    while (!feof(fp)) {
        ...
        // for nonrandom machines, just read the tools into indexs 1..n
        // no matter their tool numbers.  NB leave the spindle index 0
        // unchanged/empty.
        ...
        if(scan_old_style(buffer, toolTable, ttcomments,
                                random_toolchanger, continuous_index)) continue;
        ...
        if (!random_toolchanger && toolTable[0].toolno == toolTable[index].toolno) {
            toolTable[0] = toolTable[index];
        }
    }
