...
import emccanon
from interpreter import INTERP_ERROR, INTERP_OK, TOLERANCE_EQUAL, INTERP_EXECUTE_FINISH, INVERSE_TIME
throw_exceptions = 1
...
def prepare_epilog(self, **words):
    try:
        ...
        self.set_errormsg("T%d/prepare_epilog: %s" % (int(words['t']), e))
        return INTERP_ERROR
...
def cycle_prolog(self, **words):
    ...
    try:
        ...
        if "r" not in self.sticky_params[r.name]:
